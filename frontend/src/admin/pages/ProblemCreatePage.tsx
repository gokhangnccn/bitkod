...
export default function ProblemCreatePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.data.IsSucceeded) {
        ...
        toast.error(res.data.Message || 'Hata');
      }
    } catch (err) {
      toast.error('Hata');
      console.error(err);
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h2 className="text-2xl font-semibold mb-4">Yeni Soru Oluştur</h2>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <textarea value={description} onChange={e=>setDescription(e.target.value)} className="w-full border px-3 py-2 rounded min-h-[120px]" required />
        </div>
        <div>
